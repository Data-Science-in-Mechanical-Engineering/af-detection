...
from ..scripts.util import STANDARD_SPH_MINIMUM_RRIS, STANDARD_COAT_MINIMUM_RRIS
...
def plot_ecg(
    ...
    ax = sns.lineplot(x=time_seconds, y=ecg_signal)
    x, y = ax.get_lines()[0].get_data()
    segments = np.array([x[:-1], y[:-1], x[1:], y[1:]]).T.reshape(-1, 2, 2)

    peak_distance = np.abs(qrs_complexes[:, None] - time_indices[None, :]).min(axis=0) + 1
    ...
    if mark_peaks:
        ...
        sns.scatterplot(
            x=qrs_complexes_seconds,
            y=np.zeros_like(qrs_complexes_seconds) - 10 * marker_size,
...
if __name__ == "__main__":
    ...
    RESULT = Result.from_json(RESULTS_FOLDER / "svm rri/test-performance-cross-database.json")
    FOLDER = RESULTS_FOLDER / "misclassifications"

    plot_misclassifications(
        RESULT,
        {"SPHDataset": SPH, "COATDataset": COAT},
        {"SPHDataset": STANDARD_SPH_MINIMUM_RRIS + 1, "COATDataset": STANDARD_COAT_MINIMUM_RRIS + 1},
        FOLDER,
        lambda snapshot: f"{snapshot.setup['dataset_train']['name']}-{snapshot.setup['dataset_validate']['name']}"
    )
